    ...
    def handleDanmaku(self, danmaku):
        ...
        if danmaku.action == 3:  # online clients
            ...
            if 'info' in raw:
                ...
                pass
            elif raw['cmd'] == 'WELCOME':
                pass
            elif raw['cmd'] == 'WELCOME_GUARD':
                pass
            elif raw['cmd'] in ['SYS_GIFT']:
                pass
...
def initHandlers(roomid, config_path=None):
    if config_path:
        config = bili_config.Config(path=config_path)
    else:
